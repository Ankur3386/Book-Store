...
const AddBook = () => {
  ...
  const submit = async () => {
    try {
      ...
      formData.append("image", imageFile); // Must match backend's upload.single("image")

      const response = await axios.post("http://localhost:5000/api/v1/create-book", formData, {
        headers: {
          ...headers,
